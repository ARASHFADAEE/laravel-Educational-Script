...
@section('description', optional($course->seo)->meta_description ??
...
@section('og-description', optional($course->seo)->og_description ?? (optional($course->seo)->meta_description ??
    \Illuminate\Support\Str::limit(strip_tags($course->description), 160)))
@section('og-img', optional($course->seo)->og_image ? asset('storage/' . $course->seo->og_image) : asset('storage/' .
    $course->thumbnail))

@section('twitter-title', optional($course->seo)->twitter_title ?? $course->title)
@section('twitter-description', optional($course->seo)->twitter_description ?? (optional($course->seo)->meta_description
    ?? \Illuminate\Support\Str::limit(strip_tags($course->description), 160)))
@section('twitter-img', optional($course->seo)->twitter_image ? asset('storage/' . $course->seo->twitter_image) :
                    ...
                    <div class="bg-gradient-to-b from-white via-gray-200 to-white h-full w-full">
                        <div class="flex flex-col gap-4 h-full px-5 pb-16 pt-4 relative mx-0.5 ">
                            <div class="flex flex-col gap-1">
                                ...
                                <div
                                    class="flex flex-col gap-1 px-3 pb-2 pt-1 bg-gray-50 dark:bg-slate-800 rounded-lg -mx-1 mt-4">
                                ...
                                <div
                                    class="flex flex-col gap-1 px-3 pb-2 pt-1 bg-gray-50 dark:bg-slate-800 rounded-lg -mx-1">
                ...
                <div class="lg:basis-8/12 xl:basis-9/12 pt-4 md:pt-8 lg:pt-16 pb-96 lg:px-2 flex flex-col gap-20">
                    ...
                    <div
                        ...
                        <video class="w-full js-player">
                            <source
                                src="{{ $lesson_one && $lesson_one->video_url ? $lesson_one->video_url : ($course->video_preview ? asset('storage/' . $course->video_preview) : '#') }}"
                                type="video/mp4">
                    ...
                    <div class="flex flex-wrap sm:flex-nowrap gap-2 sm:gap-4 lg:hidden -mt-16">
                        <div
                            class="grow sm:grow-none sm:flex-1 flex flex-col gap-1 px-3 pb-2 pt-1 bg-gray-50 dark:bg-slate-800 rounded-lg">
                            <span class="text-sm font-medium text-gray-400">مدرس:</span>
                            <div class="flex items-center gap-2">
                                <svg class="size-4 fill-gray-500 dark:fill-gray-300" viewBox="0 0 448 512">
                                    <path
                        ...
                        <div class="flex flex-col gap-10">
                            <div
                                ...
                                <img src="{{ asset('storage/' . $course->user->avatar) }}" alt=""
                                    class="rounded-full size-14 md:size-16">
                                <div
                            ...
                            @foreach ($chapters as $chapter)
                                <div class="accordion group border border-gray-200 dark:border-slate-700 rounded-lg sm:rounded-xl hover:border-gray-500 transition-all"
                                    ...
                                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-1.5 sm:gap-3 md:gap-4 px-4 py-3 sm:py-5 relative header cursor-pointer text-base sm:text-lg font-semibold sm:font-bold leading-loose transition-all"
                                        ...
                                        <span
                                            class="shrink-0 rounded-full text-sm sm:text-lg px-2.5 py-0.5 bg-blue-600 text-white sm:min-w-28 text-center transition-all">
                                            فصل {{ $loop->iteration }}
                                        </span> {{ $chapter->title }}
                                    ...
                                    <div class="content transition-all text-base leading-9 text-gray-500 dark:text-gray-300 mx-4 flex flex-col items-stretch overflow-hidden"
                                        ...
                                        <ul class="flex flex-col gap-1 pt-1 md:pt-5 mb-4 border-t border-t-gray-100">
                                            @foreach ($chapter->lessons as $lesson)
                                                <li
                                                    class="block sm:flex items-center gap-3 md:gap-6 relative rounded-lg px-2.5 sm:px-4 py-1.5 sm:py-3 text-base md:text-lg font-semibold cursor-pointer transition-all hover:bg-primary-50">
                                                    <span
                                                        class="sm:ml-2 sm:min-w-20 text-xs sm:text-sm md:text-lg font-medium sm:font-bold text-blue-600">
                                                        جلسه {{ $loop->iteration }}
                                                    </span>
                                                    <div class="text-black dark:text-white">{{ $lesson->title }}</div>
                                                    @if ($lesson->duration)
                                                        <small
                                                            class="mr-auto text-xs sm:text-sm font-light md:font-medium text-gray-400 hidden sm:block">{{ $lesson->duration }}
                                                            دقیقه</small>
                                                    @endif
                                                </li>
                                            @endforeach
                    ...
                    <div class="flex flex-col gap-8 text-black dark:text-white">
                        ...
                        <div class="grid xl:grid-cols-2 gap-4">
                            <div
                                class="flex flex-col gap-2 py-4 px-5 border-2 border-black dark:border-slate-700 rounded-xl text-black dark:text-white">
                                <span
                                    class="text-2xl font-extrabold leading-loose flex items-center gap-3 text-black dark:text-white">
                                ...
                                <small class="text-base leading-8 text-black dark:text-white">گارانتی بازگشت وجه تا یک
                                    هفته،
                                    ثبت&zwnj;نام شما در
                            ...
                            <div
                                ...
                                <span
                                    ...
                                    <svg viewBox="0 0 24 24" fill="none"
                                        class="size-10 stroke-black dark:stroke-white" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <small class="text-base leading-8 text-black dark:text-white">با اعتماد ۵۰ هزار دانشجو در
                                    دو دهه
                                    فعالیت، مسیر یادگیری شما را با اطمینان همراهی می&zwnj;کنیم.</small>
                            </div>
                            <div
                                class="flex flex-col gap-2 py-4 px-5 border-2 border-black dark:border-slate-700 rounded-xl">
                                ...
                                <small class="text-base leading-8 text-black dark:text-white">از مدرسین متخصص در بهترین
                                    شرکت&zwnj;های
                                    ایران مهارت
                            ...
                            <div
                                ...
                                <span class="text-2xl font-extrabold leading-loose flex items-center gap-3">
                                    ...
                                    مشاوره اختصاصی</span>
                                <small class="text-base leading-8 text-black dark:text-white">برای شروع یادگیری و یافتن
                                    بهترین مسیر
                                    شغلی،
                    ...
                    <div class="flex flex-col gap-4 sm:gap-8" id="pay">
                        ...
                        <div
                            class="flex flex-col gap-2 mt-3 p-6 sm:p-8 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-xl sm:rounded-2xl">
                            <h3 class="block text-2xl font-black">
                                <span
                                    class="text-blue-600">{{ Auth::check() ? Auth::user()->name : 'کاربر عزیز' }}</span>،
                                سلام
                            </h3>
                            <span class="text-lg font-medium text-gray-400 mt-6 mb-3">برای دسترسی به محتوای دوره، روی دکمه
                                زیر کلیک کنید</span>
                    ...
                    <form action="{{ route('course.checkout.zibal', $course->id) }}" method="POST" class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="text" name="first_name" value="{{ $defaultFirstName }}"
                            ...
                            <div>
                                ...
                                <input type="text" name="last_name" value="{{ $defaultLastName }}"
                                    class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-800 focus:border-blue-500 focus:outline-none">
                                @error('last_name')
                                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div>
                            ...
                            <input type="text" name="phone" value="{{ $defaultPhone }}" placeholder="09xxxxxxxxx"
                                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-800 focus:border-blue-500 focus:outline-none">
                            @error('phone')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
