<div class="flex flex-col divide-y divide-border">
    ...
    <div class="w-full space-y-2 py-3" x-data="{ open: true }">
        ...
        <button type="button"
            ...
            x-bind:class="open ? 'text-primary' : 'text-foreground'" x-on:click="open = !open">
        ...
        <div class="bg-secondary rounded-2xl relative p-3" x-show="open">
            <div class="space-y-2">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="type" value="all" x-model="type"
                        class="form-radio !ring-0 !ring-offset-0 bg-border border-0" />
                    <span class="text-sm text-muted">همه</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="type" value="free" x-model="type"
                        class="form-radio !ring-0 !ring-offset-0 bg-border border-0" />
                    <span class="text-sm text-muted">رایگان</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="type" value="cash" x-model="type"
                        class="form-radio !ring-0 !ring-offset-0 bg-border border-0" />
    ...
    <div class="w-full space-y-2 py-3" x-data="{ open: false }">
        ...
        <div class="bg-secondary rounded-2xl relative p-3" x-show="open">
            <div class="space-y-2">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input value="all" type="radio" name="category" x-model="category"
                        class="form-radio !ring-0 !ring-offset-0 bg-border border-0" />
                    <span class="text-sm text-muted">همه دسته‌ها</span>
                </label>
                @foreach ($categories as $cat)
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input value="{{ $cat->slug }}" type="radio" name="category" x-model="category"
                            class="form-radio !ring-0 !ring-offset-0 bg-border border-0" />
